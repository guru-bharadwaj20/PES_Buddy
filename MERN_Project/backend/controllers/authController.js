...
import { handleError, validationError, unauthorizedError, forbiddenError, notFoundError } from "../utils/errorHandler.js";
...
export const register = async (req, res) => {
	try {
		...
		if (!name || !srn || !password) return validationError(res, "Name, SRN and password required");
		const exists = await User.findOne({ srn });
		if (exists) return validationError(res, "User with this SRN already exists");
...
export const login = async (req, res) => {
	try {
		...
		if (!srn || !password) return validationError(res, "SRN and password required");
		const user = await User.findOne({ srn });
		if (!user) return unauthorizedError(res, "Invalid credentials");
		
		// Check if admin login is requested and user has admin role
		if (isAdmin && user.role !== 'admin') {
			return forbiddenError(res, "Access denied. Admin privileges required.");
		}
		
		// Check if customer login but user is admin (should use admin portal)
		if (!isAdmin && user.role === 'admin') {
			return forbiddenError(res, "Please use admin portal to login.");
		}
		
		const isMatch = await user.comparePassword(password);
		if (!isMatch) return unauthorizedError(res, "Invalid credentials");
		const token = generateToken(user._id);
...
export const forgotPassword = async (req, res) => {
	try {
		...
		if (!user) return notFoundError(res, "No user found with this email");
