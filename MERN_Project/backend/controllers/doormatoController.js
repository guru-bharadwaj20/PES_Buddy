...
export const createOrder = async (req, res) => {
	try {
		...
		if (!items || !Array.isArray(items) || items.length === 0) return validationError(res, "No items in order");
		...
		if (orderItems.length === 0) return validationError(res, "No valid items found");
